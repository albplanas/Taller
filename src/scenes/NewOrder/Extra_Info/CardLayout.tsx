...
export const CardLayout=(props)=>{
    return  <Layout style={[styles.layout,{padding:1,maxWidth:props.width,minHeight:450,maxHeight:450}]} level='4'>
                <Card style={[styles.card,{width:'100%'}]} header={()=>props.type==="corner"?<HeaderCard icon={props.iconHead} 
                                title={props.title}/>:<HeaderCenterCard icon={props.iconHead} {...props}/>} status='success'>
                    <Layout style={styles.rowContainer}>
                            {props.type==="corner"?<LeftList data={props.data} setData={(obj)=>{props.setData(obj)}}/>:
                            <CenterList  {...props}/>}
                    </Layout>
                </Card>
...
const HeaderCard=(props)=>{
    return <CardHeader title={props.title}>
              <Button icon={props.icon} 
                      appearance="ghost" size="giant" status={"basic"} textStyle={{fontSize:20,fontFamily:"sans-serif-thin"}}  >
                      {props.title}
              </Button>
            </CardHeader>
  }
